...
function renderCurrentScreen(){if(state.screen==='orders')renderOrders();else if(state.screen==='active')renderActive();else if(state.screen==='late')renderLate();else if(state.screen==='user')renderUserBoard();else renderHome();}
...
function renderHome(){
  ...
  qs('#content').innerHTML=`${toolbar()}<div class="grid cols-3"><div class="card metric"><h3>Metas da semana</h3><b>${week.length}</b></div><div class="card metric"><h3>Metas ativas</h3><b>${active.length}</b></div><div class="card metric"><h3>Em atraso</h3><b>${late.length}</b></div></div><section class="card metas2-week-list"><div class="section-head"><div><h3>Semana selecionada</h3><p class="muted">Controle semanal de metas. Os prazos individuais continuam visíveis dentro de cada card.</p></div></div>${week.length?`<div class="metas2-card-grid">${week.map(metaCardHtml).join('')}</div>`:'<div class="empty">Nenhuma meta programada para esta semana.</div>'}</section><section class="card metas2-active-home"><div class="section-head"><div><h3>Metas ativas</h3><p class="muted">Até 15 metas em aberto para acompanhamento rápido.</p></div><button type="button" class="btn small secondary" id="metaActiveAll">Ver todas</button></div>${active.length?`<div class="metas2-card-grid">${active.slice(0,15).map(metaCardHtml).join('')}</div>`:'<div class="empty compact">Nenhuma meta ativa.</div>'}</section><section class="card metas2-late-home"><div class="section-head"><div><h3>Metas atrasadas</h3><p class="muted">Até 15 metas vencidas para acompanhamento rápido.</p></div><button type="button" class="btn small secondary" id="metaLateAll">Ver todas</button></div>${late.length?`<div class="metas2-card-grid">${late.slice(0,15).map(metaLateCardHtml).join('')}</div>`:'<div class="empty compact">Nenhuma meta atrasada.</div>'}</section><h3 class="section-title">Colaboradores</h3><div class="metas-emp-grid">${employees.map(u=>{const ms=metasForUser(u.id);const act=ms.filter(m=>!['Concluído','Cancelado'].includes(m.status));const done=ms.filter(m=>m.status==='Concluído');return `<button type="button" class="card metas-emp-card" data-meta-user="${u.id}"><div class="metas-emp-head"><strong>${esc(u.name)}</strong><span class="badge">${esc(u.type)}</span></div><div class="metas-emp-stats"><div><b>${act.length}</b><span>Ativas</span></div><div><b>${done.length}</b><span>Concluídas</span></div><div><b>${ms.length}</b><span>Histórico</span></div></div></button>`}).join('')}</div>`;
  wireToolbar();qs('#metaActiveAll')?.addEventListener('click',()=>{state.screen='active';renderActive()});qs('#metaLateAll')?.addEventListener('click',()=>{state.screen='late';renderLate()});wireMetaCards();qsa('[data-meta-user]').forEach(b=>b.onclick=()=>{state.selectedUser=b.dataset.metaUser;state.screen='user';renderUserBoard()});
}
function metaLateCardHtml(m){const resp=metaResponsibles(m.id).map(userName).join(', ')||'Sem responsável';const action=conclusionActionHtml(m);return `<button type="button" class="metas2-card metas2-card-late" data-meta-card="${m.id}"><div class="metas2-card-top"><strong>${esc(m.titulo)}</strong><span class="badge danger">Atrasada</span></div><span class="muted metas2-assoc">${esc(associationLabel(m))}</span><p>${esc(m.observacoes||'Sem observações.')}</p><div class="metas2-card-foot"><span>${esc(sectorLabel(m.setor_id))}</span><span class="meta-late-deadline">Prazo ${brDate(m.prazo)}</span></div><small>${esc(resp)}</small>${action?`<div class="meta-conclusion-row">${action}</div>`:''}</button>`;}
...
function renderActive(){
  ...
  qs('#content').innerHTML=`<div class="toolbar"><button class="btn ghost" id="metaActiveBack">← Voltar</button><div class="right">${canManageSectors()?'<button class="btn secondary" id="metaNewSector">+ Setor</button>':''}${canManageMeta()?'<button class="btn" id="metaActiveNew">+ Nova Meta</button>':''}</div></div><section class="card metas-user-board"><div class="section-head"><div><h3>Metas em aberto</h3><p class="muted">Organizadas por responsável para facilitar o acompanhamento.</p></div></div>${userColumnsHtml(active)}</section>${sectorPanelHtml(active)}`;
...
function renderLate(){
  state.screen='late';B().title('Metas Atrasadas');const late=visibleMetas().filter(m=>!['Concluído','Cancelado'].includes(m.status)&&m.prazo&&m.prazo<today());
  qs('#content').innerHTML=`<div class="toolbar"><button class="btn ghost" id="metaLateBack">← Voltar</button><div class="right">${canManageSectors()?'<button class="btn secondary" id="metaLateNewSector">+ Setor</button>':''}${canManageMeta()?'<button class="btn" id="metaLateNew">+ Nova Meta</button>':''}</div></div><section class="card metas-user-board metas-user-board-late"><div class="section-head"><div><h3>Metas atrasadas</h3><p class="muted">Organizadas por responsável, com os prazos vencidos em destaque.</p></div></div>${userColumnsHtml(late,true)}</section>${sectorPanelHtml(late)}`;
  qs('#metaLateBack').onclick=renderHome;qs('#metaLateNewSector')?.addEventListener('click',()=>sectorModal());qs('#metaLateNew')?.addEventListener('click',()=>metaModal());wireMetaCards();wireSectorPanel();
}
