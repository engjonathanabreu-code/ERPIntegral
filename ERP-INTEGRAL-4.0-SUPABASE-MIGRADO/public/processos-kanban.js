...
async function openDetail(id){
  ...
  el.innerHTML=`<section class="modal process-modal"><header class="modal-head"><div><h3>${esc(p.nucleo)}</h3><small>${esc(p.municipio)} / ${esc(p.estado)}</small></div><button class="btn icon ghost" data-close>×</button></header><div class="modal-body"><div class="process-detail-grid"><label>Etapa<select id="pdStage">${STAGES.map(s=>`<option ${s===p.etapa_atual?'selected':''}>${esc(s)}</option>`).join('')}</select></label><label>Responsável<select id="pdOwner"><option value="">Não definido</option>${profiles.map(u=>`<option value="${u.id}" ${u.id===p.responsavel_id?'selected':''}>${esc(u.nome)}</option>`).join('')}</select></label><label>Prazo<input id="pdDeadline" type="date" value="${p.prazo||''}">${p.prazo?'<button type="button" class="btn small secondary" id="processAddCalendar">Adicionar ao calendário</button>':''}</label><label>Prioridade<select id="pdPriority">${['Baixa','Normal','Alta','Urgente'].map(x=>`<option ${x===p.prioridade?'selected':''}>${x}</option>`).join('')}</select></label><label class="full">Pendência<textarea id="pdPendency" rows="2">${esc(p.pendencia||'')}</textarea></label></div><div class="process-detail-actions"><button id="pdSave" class="btn">Salvar alterações</button></div><div class="process-detail-sections"><button type="button" class="process-section-toggle" data-process-section="crm" aria-expanded="false"><span>Andamentos importados do CRM</span><span class="process-section-count">${arr.length}</span><span class="process-section-chevron">⌄</span></button><div class="process-section-panel" data-process-panel="crm" hidden><div class="process-timeline">${arr.map(a=>`<article><b>${esc(a.status)}</b><span>${esc(a.status_operacional||'')}</span><small>${brDate(a.data_atualizacao)}${a.previsao?' · previsão '+brDate(a.previsao):''}</small>${a.descricao_cliente?`<p>${esc(a.descricao_cliente)}</p>`:''}${a.observacao_interna?`<p class="internal">Interno: ${esc(a.observacao_interna)}</p>`:''}</article>`).join('')||'<div class="empty">Nenhum andamento registrado no CRM.</div>'}</div></div><button type="button" class="process-section-toggle" data-process-section="history" aria-expanded="false"><span>Histórico de etapas no ERP</span><span class="process-section-count">${history.length}</span><span class="process-section-chevron">⌄</span></button><div class="process-section-panel" data-process-panel="history" hidden><div class="process-timeline compact">${history.map(h=>`<article><b>${esc(h.etapa_anterior||'Início')} → ${esc(h.etapa_nova)}</b><small>${new Date(h.created_at).toLocaleString('pt-BR')}</small>${h.observacao?`<p>${esc(h.observacao)}</p>`:''}</article>`).join('')||'<div class="empty">Nenhuma movimentação manual registrada.</div>'}</div></div></div></div></section>`;
  el.querySelector('.modal-body').dataset.collabEntityType='processo';el.querySelector('.modal-body').dataset.collabEntityId=id;document.body.appendChild(el);window.ERPCollaboration?.bindPersonalButton(el.querySelector('#processAddCalendar'),'processo:'+id);el.querySelector('[data-close]').onclick=()=>el.remove();el.onclick=e=>{if(e.target===el)el.remove()};el.querySelectorAll('[data-process-section]').forEach(button=>button.onclick=()=>{const panel=el.querySelector(`[data-process-panel="${button.dataset.processSection}"]`),open=panel.hidden;panel.hidden=!open;button.setAttribute('aria-expanded',String(open));});
