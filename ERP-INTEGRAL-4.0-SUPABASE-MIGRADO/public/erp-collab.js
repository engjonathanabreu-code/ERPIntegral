(() => {
  ...
  function renderCalendar(){
    ...
    const events=state.events.filter(e=>(!state.agenda||e.agenda_id===state.agenda)&&(state.filter!=='pessoal'||e.created_by===me().id||e.participantes.includes(me().id)||mine.has(`evento:${e.id}`)));
    const deadlines=state.agenda?[]:state.deadlines.filter(e=>state.filter!=='pessoal'||e.participantes.includes(me().id)||mine.has(e.chave));
    ...
    $('#content').innerHTML=`<div class="collab-toolbar"><h3>${date(p.start)} a ${date(p.end)}</h3><button class="btn secondary" data-move="-1" aria-label="Período anterior">←</button><button class="btn secondary" id="collabToday">Hoje</button><button class="btn secondary" data-move="1" aria-label="Próximo período">→</button><select aria-label="Visualização" id="collabMode">${['quinzenal','mensal','trimestral'].map(x=>`<option value="${x}" ${state.mode===x?'selected':''}>${x[0].toUpperCase()+x.slice(1)}</option>`).join('')}</select></div>
      ...
      <p class="collab-hint">Horários de Brasília. Cada prazo aparece diariamente até seu vencimento. Clique em um evento para ver detalhes.</p>
  ...
  async function eventDetail(id){
    ...
    bind('#collabOpenLinked',()=>openEntity(ev.entidade_tipo,ev.entidade_id));bind('#collabAddPersonal',async b=>{await action('pessoal',{chave:`evento:${id}`});b.textContent='Adicionado';});
  ...
  function deadlineDetail(ev){
    ...
    bind('#collabOpenLinked',()=>openEntity(ev.entidade_tipo,ev.entidade_id));bind('#collabAddPersonal',async b=>{await action('pessoal',{chave:ev.chave});b.textContent='Adicionado';});bind('#collabSaveColor',async()=>{await action('cor_prazo',{chave:ev.chave,cor:$('#collabColor').value});closeDialog();await loadCalendar();renderCalendar();});
  }
  ...
  async function notifications(){await loadReference();await refreshNotifications();dialog('Notificações',`<div class="collab-list">${state.notifications.sort((a,b)=>Number(a.lida)-Number(b.lida)).map((n,i)=>`<article class="collab-notice ${n.lida?'':'unread'}"><p>${esc(n.titulo)}</p>${n.prazo?`<small>Prazo: ${date(n.prazo)}</small>`:''}<div class="collab-actions"><button class="btn secondary" data-notice="${i}">Abrir</button>${n.evento_id||n.tipo==='prazo'||n.tipo==='vinculo'?`<button class="btn secondary" data-personal-notice="${i}">Adicionar à minha agenda</button>`:''}${!n.lida?`<button class="btn secondary" data-read-notice="${i}">Marcar como lida</button>`:''}</div></article>`).join('')||'<p>Você está em dia. Nenhuma notificação.</p>'}</div>`);
    ...
    bind('[data-personal-notice]',async b=>{const n=state.notifications[Number(b.dataset.personalNotice)];const key=n.evento_id?`evento:${n.evento_id}`:n.chave.replace(/^vinculo:/,'').replace(/^prazo:/,'').replace(/:\d{4}-\d{2}-\d{2}$/,'');await action('pessoal',{chave:key});b.textContent='Adicionado';});
  }
  ...
  async function chatForm(){await loadReference();const group=state.tab==='grupo';formDialog(group?'Novo grupo de trabalho':'Nova conversa',`${group?'<label>Nome do grupo<input name="titulo" required maxlength="200"></label>':''}<label>${group?'Participantes (Ctrl ou ⌘ para selecionar vários)':'Usuário'}<select name="participantes" ${group?'multiple':''} required>${userOptions([])}</select></label>${group?`<label>Registro associado<select name="vinculo" required>${linkOptions('',true)}</select></label>`:''}`,async fd=>{const r=await action('conversa',{tipo:group?'grupo':'direto',titulo:fd.get('titulo')||'Conversa direta',participantes:fd.getAll('participantes'),...splitLink(fd.get('vinculo'))});closeDialog();state.chat=r.id;await loadChats();renderChat();});}
  ...
  window.ERPCollaboration={open:openPage,history,eventDetail};reconcile();
})();
