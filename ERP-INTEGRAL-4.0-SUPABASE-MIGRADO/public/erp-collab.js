(() => {
  ...
  let state={userId:null,page:null,mode:'mensal',anchor:today(),filter:'todos',agenda:'',events:[],deadlines:[],agendas:[],users:[],links:[],colors:[],personal:[],hidden:[],notifications:[],chats:[],chat:null,tab:'direto',messages:[]};
  ...
  function renderCalendar(){
    ...
    const events=state.events.filter(e=>!hidden.has(`evento:${e.id}`)&&(!state.agenda||e.agenda_id===state.agenda)&&(state.filter!=='pessoal'||mine.has(`evento:${e.id}`)));
    const deadlines=state.agenda?[]:state.deadlines.filter(e=>!hidden.has(e.chave)&&['meta','plano','processo'].includes(e.entidade_tipo)&&(state.filter!=='pessoal'||mine.has(e.chave)));
