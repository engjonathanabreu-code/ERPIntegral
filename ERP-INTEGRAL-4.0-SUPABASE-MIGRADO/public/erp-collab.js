(() => {
  ...
  let state={userId:null,page:null,mode:'mensal',anchor:today(),filter:'todos',agenda:'',userFilter:'',events:[],deadlines:[],agendas:[],users:[],links:[],colors:[],personal:[],hidden:[],notifications:[],chats:[],chat:null,tab:'direto',messages:[]};
  ...
  async function loadCalendar(){
    ...
    const result=await Promise.all([rows('erp_eventos',q=>q.lt('inicio',`${C.move(p.end,'quinzenal',1)}T00:00:00-03:00`).gt('fim',`${p.start}T00:00:00-03:00`)),rows('erp_prazos'),rows('erp_cores_prazos'),rows('erp_agenda_pessoal'),rows('erp_calendario_ocultos')]);
    [state.events,state.deadlines,state.colors,state.personal,state.hidden]=result;
  }
  ...
  function renderCalendar(){
    ...
    const assigned=e=>!state.userFilter||(e.participantes||[]).includes(state.userFilter);
    ...
    $('#content').innerHTML=`<div class="collab-toolbar"><h3>${date(p.start)} a ${date(p.end)}</h3><button class="btn secondary" data-move="-1" aria-label="Período anterior">←</button><button class="btn secondary" id="collabToday">Hoje</button><button class="btn secondary" data-move="1" aria-label="Próximo período">→</button><select aria-label="Visualização" id="collabMode">${['quinzenal','mensal','trimestral'].map(x=>`<option value="${x}" ${state.mode===x?'selected':''}>${x[0].toUpperCase()+x.slice(1)}</option>`).join('')}</select></div>
      <div class="collab-toolbar"><select id="collabFilter" aria-label="Filtrar eventos"><option value="todos">Todos os eventos acessíveis</option><option value="pessoal" ${state.filter==='pessoal'?'selected':''}>Minha agenda</option></select><select id="collabAgenda" aria-label="Agenda de ativo"><option value="">Todas as agendas</option>${state.agendas.map(a=>`<option value="${a.id}" ${a.id===state.agenda?'selected':''}>${esc(a.nome)}</option>`).join('')}</select><button class="btn" id="collabNewEvent">+ Novo evento</button>${admin()?'<button class="btn secondary" id="collabNewAgenda">+ Agenda de ativo</button>':''}${admin()?`<select id="collabUserFilter" aria-label="Filtrar por usuário"><option value="">Todos os usuários</option>${state.users.map(u=>`<option value="${u.id}" ${state.userFilter===u.id?'selected':''}>${esc(u.nome)}</option>`).join('')}</select>`:''}<button class="btn secondary" id="collabHidden">Ocultados</button><button class="btn secondary" id="collabRefresh">Atualizar</button></div>
    ...
    $('#collabFilter').onchange=e=>{state.filter=e.target.value;renderCalendar();};$('#collabAgenda').onchange=e=>{state.agenda=e.target.value;renderCalendar();};$('#collabUserFilter')&&( $('#collabUserFilter').onchange=e=>{state.userFilter=e.target.value;renderCalendar();});
    bind('#collabHidden',()=>hiddenItemsDialog());bind('#collabRefresh',async()=>{await loadReference();await loadCalendar();renderCalendar();});
  ...
  async function eventDetail(id){
    ...
    bind('#collabHideItem',async b=>{const visible=await toggleHidden(`evento:${id}`);b.textContent=visible?'Ocultar do meu calendário':'Mostrar no meu calendário';});bind('[data-rsvp]',async b=>{await action('resposta',{id,resposta:b.dataset.rsvp});await eventDetail(id);});
  ...
  async function toggleHidden(key){const own=state.hidden.some(x=>x.chave===key);const q=sb().from('erp_calendario_ocultos');const r=own?await q.delete().eq('usuario_id',me().id).eq('chave',key):await q.insert({usuario_id:me().id,chave:key});if(r.error)throw r.error;if(own)state.hidden=state.hidden.filter(x=>x.chave!==key);else state.hidden.push({usuario_id:me().id,chave:key});return own;}
  function hiddenItemsDialog(){const items=state.hidden.map(h=>{const deadline=state.deadlines.find(x=>x.chave===h.chave),event=state.events.find(x=>`evento:${x.id}`===h.chave);return {key:h.chave,title:deadline?.titulo||event?.titulo||h.chave};});dialog('Itens ocultados',`<p class="collab-hint">Esses itens estão ocultos apenas no seu calendário.</p><div class="collab-list">${items.map(x=>`<article class="collab-notice"><p>${esc(x.title)}</p><button class="btn secondary" data-restore-hidden="${esc(x.key)}">Reativar visualização</button></article>`).join('')||'<p>Nenhum item ocultado.</p>'}</div>`);bind('[data-restore-hidden]',async b=>{await toggleHidden(b.dataset.restoreHidden);closeDialog();if(state.page==='calendario')renderCalendar();});}
  ...
  function deadlineDetail(ev){
    if(!ev)return;const assigned=(ev.participantes||[]).map(userName).map(esc).join(', ')||'Sem responsável atribuído';dialog(ev.titulo,`<p>${labels[ev.entidade_tipo]} · Prazo: ${date(ev.fim)}</p><p><b>Atribuído a:</b> ${assigned}</p><div class="collab-actions"><button class="btn" id="collabOpenLinked">Abrir registro</button><button class="btn secondary" id="collabAddPersonal">Adicionar à minha agenda</button><button class="btn secondary" id="collabHideItem">Ocultar do meu calendário</button></div>${admin()?'<div class="collab-actions"><label>Cor <input type="color" id="collabColor" value="'+(state.colors.find(x=>x.chave===ev.chave)?.cor||'#2563eb')+'"></label><button class="btn secondary" id="collabSaveColor">Salvar cor</button></div>':''}`);
    bind('#collabOpenLinked',()=>openEntity(ev.entidade_tipo,ev.entidade_id));bind('#collabAddPersonal',async b=>{const added=await togglePersonal(ev.chave);b.textContent=added?'Remover da minha agenda':'Adicionar à minha agenda';});bind('#collabHideItem',async b=>{const visible=await toggleHidden(ev.chave);b.textContent=visible?'Ocultar do meu calendário':'Mostrar no meu calendário';});bind('#collabSaveColor',async()=>{await action('cor_prazo',{chave:ev.chave,cor:$('#collabColor').value});closeDialog();await loadCalendar();renderCalendar();});
  }
