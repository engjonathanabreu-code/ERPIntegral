...
async function syncRemoteDB(){
  ...
  try{
    ...
    if(canManageCore()){
      ...
      const steps=db.plans.flatMap(p=>p.steps.map((s,i)=>({id:s.id,plano_id:p.id,titulo:s.title,inicio_prazo:s.startDeadline||null,prazo:s.deadline||null,status:planStepStatusToRemote(s.status),observacoes:s.notes||null,ordem:i})));
