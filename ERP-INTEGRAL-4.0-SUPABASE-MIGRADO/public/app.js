...
async function loadRemoteDB(){
  ...
  db={version:3,
    ...
    projects:projects.map(x=>({id:x.id,icon:x.icone||'',name:x.nome,clientId:x.cliente_id||'',type:x.tipo_servico,manager:x.responsavel||'',status:x.status,start:x.data_inicio||'',deadline:x.prazo_final||'',contractValue:Number(x.valor_contrato||0),notes:x.observacoes||'',createdAt:(x.created_at||'').slice(0,10),stages:stages.filter(s=>s.projeto_id===x.id).map(s=>({id:s.id,icon:s.icone||'',name:s.titulo,owner:s.descricao||'',deadline:s.prazo||'',progress:Number(s.progresso||0),weight:Number(s.peso||0),status:projectStageStatusFromRemote(s.status)}))})),
    ...
    plans:plans.map(x=>({id:x.id,icon:x.icone||'',title:x.titulo,projectId:x.projeto_id||'',status:planStatusFromRemote(x.status),createdAt:(x.created_at||'').slice(0,10),comments:comments.filter(c=>c.plano_id===x.id).map(c=>({id:c.id,authorId:c.autor_id,text:c.texto,createdAt:c.created_at})),steps:steps.filter(s=>s.plano_id===x.id).map(s=>({id:s.id,icon:s.icone||'',title:s.titulo,startDeadline:s.inicio_prazo||'',deadline:s.prazo||'',status:planStepStatusFromRemote(s.status),notes:s.observacoes||'',createdAt:s.created_at||'',completedAt:s.concluido_em||'',responsibleIds:responsibles.filter(r=>r.etapa_id===s.id).map(r=>r.usuario_id),deliverables:deliverables.filter(d=>d.etapa_id===s.id).map(d=>({id:d.id,text:d.titulo,done:!!d.concluido}))}))}))
  };
...
async function syncRemoteDB(){
  ...
  try{
    ...
    if(canManageCore()){
      ...
      await upsertRows('projetos',db.projects.map(p=>({id:p.id,icone:p.icon||null,cliente_id:p.clientId||null,nome:p.name,tipo_servico:p.type,responsavel:p.manager||null,status:projectStatusToRemote(p.status),data_inicio:p.start||null,prazo_final:p.deadline||null,valor_contrato:Number(p.contractValue||0),observacoes:p.notes||null,created_by:currentUser.id})));
      await deleteMissing('projetos',db.projects.map(x=>x.id));
      const projectStages=db.projects.flatMap(p=>p.stages.map((s,i)=>({id:s.id,icone:s.icon||null,projeto_id:p.id,titulo:s.name,descricao:s.owner||null,peso:Number(s.weight||0),progresso:Number(s.progress||0),prazo:s.deadline||null,status:projectStageStatusToRemote(s.status),ordem:i})));
      await upsertRows('etapas_projeto',projectStages);await deleteMissing('etapas_projeto',projectStages.map(x=>x.id));
      await upsertRows('planos_trabalho',db.plans.map(p=>({id:p.id,icone:p.icon||null,projeto_id:p.projectId||null,titulo:p.title,status:planStatusToRemote(p.status),created_by:currentUser.id})));
      await deleteMissing('planos_trabalho',db.plans.map(x=>x.id));
      const steps=db.plans.flatMap(p=>p.steps.map((s,i)=>({id:s.id,icone:s.icon||null,plano_id:p.id,titulo:s.title,inicio_prazo:s.startDeadline||null,prazo:s.deadline||null,status:planStepStatusToRemote(s.status),observacoes:s.notes||null,ordem:i})));
...
function renderProgress(){
  ...
  const sections=typeOrder.filter(t=>groups[t]?.length).map(type=>{
    ...
    return `<section class="progress-type-group ${isCollapsed?'collapsed':''}" data-progress-group="${esc(type)}"><button type="button" class="progress-type-head" data-toggle-progress-group="${esc(type)}" aria-expanded="${isCollapsed?'false':'true'}"><div class="progress-type-title"><span class="progress-type-chevron">⌄</span><div><strong>${esc(type)}</strong><small>${list.length} projeto${list.length===1?'':'s'} · média ${avg}%</small></div></div><div class="progress-type-highlight"><span>Mais adiantado</span><b>${esc(leader.p.name)}</b><em>${leader.pct}%</em></div></button><div class="progress-type-body"><div class="table-wrap progress-type-table-wrap"><table class="table progress-table"><thead><tr><th>Projeto</th><th>Status</th><th>Etapas</th><th>Progresso</th></tr></thead><tbody>${list.map(({p,total,done,pct})=>`<tr><td><button class="project-link" data-project="${p.id}" ${window.ERPVisualIcons?.attributes(p.icon)||''}>${esc(p.name)}</button></td><td>${statusBadge(p.status||'Ativo')}</td><td><span class="progress-stage-count">${done}/${total}</span></td><td><div class="progress-cell"><div class="progress"><i style="width:${pct}%"></i></div><b>${pct}%</b></div></td></tr>`).join('')}</tbody></table></div></div></section>`;
  }).join('');
...
function renderProjectDetail(){
  ...
  <section class="card project-hero"><div><div class="project-kicker">${esc(p.type)}</div><h3 ${window.ERPVisualIcons?.attributes(p.icon)||''}>${esc(p.name)}</h3><p>${esc(p.notes||'Sem observações cadastradas.')}</p></div>${statusBadge(p.status)}</section>
...
function planSummaryRow(p){
  ...
  return `<section class="card plan-index-row" data-open-plan="${p.id}" tabindex="0" role="button"><div class="plan-index-primary"><h3 ${window.ERPVisualIcons?.attributes(p.icon)||''}>${esc(p.title)}</h3><span class="muted">${esc(project?.name||'Sem projeto')}</span></div><div class="plan-index-stage"><small>Última etapa em aberto</small>${current?`<strong ${window.ERPVisualIcons?.attributes(current.icon)||''}>${esc(current.title)}</strong><span><span class="badge">${esc(current.status||'Pendente')}</span>${current.deadline?` <span class="badge ${deadlineClass}">Prazo ${brDate(current.deadline)}</span>`:''}</span>`:'<strong>Nenhuma etapa em aberto</strong><span class="muted">Sem pendências cadastradas</span>'}</div><div class="plan-index-progress"><small>Progresso dos entregáveis</small><strong>${pct}%</strong><div class="progress"><i style="width:${pct}%"></i></div></div><div class="plan-index-status"><span class="badge ${p.status==='Concluído'?'ok':''}">${esc(p.status)}</span><span class="plan-index-open">Abrir plano →</span></div>${canManageCore()?`<div class="actions plan-index-actions"><button class="btn icon secondary" data-edit-plan="${p.id}" title="Editar plano">✎</button><button class="btn icon danger" data-del-plan="${p.id}" title="Excluir plano">×</button></div>`:''}</section>`;
}
const STEP_OBSERVATIONS_PREFIX='__ERP_STEP_OBSERVATIONS_V1__';
function stepObservationRecords(notes){
  const raw=String(notes||'');
  if(raw.startsWith(STEP_OBSERVATIONS_PREFIX)){try{const rows=JSON.parse(raw.slice(STEP_OBSERVATIONS_PREFIX.length));return Array.isArray(rows)?rows:[]}catch{}}
  return raw.trim()?[{id:'legacy',text:raw,author:'Registro anterior',createdAt:''}]:[];
}
function encodeStepObservations(rows){return STEP_OBSERVATIONS_PREFIX+JSON.stringify(rows);}
function stepObservationsHtml(s){
  const rows=stepObservationRecords(s.notes);
  const history=rows.length?rows.map(row=>`<article class="step-observation-entry"><div class="step-observation-meta"><strong>${esc(row.author||'Usuário')}</strong><time>${row.createdAt?new Date(row.createdAt).toLocaleString('pt-BR'):'Data não registrada'}</time></div><p>${esc(row.text||'').replace(/\n/g,'<br>')}</p></article>`).join(''):'<div class="muted">Nenhuma observação registrada.</div>';
  return `<div class="step-observation-history">${history}</div>`;
}
function addStepObservation(pid,sid,text){
  const p=db.plans.find(x=>x.id===pid),s=p?.steps.find(x=>x.id===sid),value=String(text||'').trim();
  if(!s||!value||!canEditStep(s))return;
  const rows=stepObservationRecords(s.notes);
...
function wirePlanEvents(){
  ...
  $$('[data-toggle-step]').forEach(b=>b.onclick=()=>{const step=b.closest('.step');step.classList.toggle('collapsed');b.setAttribute('aria-expanded',String(!step.classList.contains('collapsed')))});
  $$('[data-step-status]').forEach(s=>s.onchange=()=>updateStepField(s.dataset.plan,s.dataset.step,'status',s.value));
  $('[data-add-step-observation]').forEach(b=>b.onclick=()=>{const input=b.closest('.step-observation-compose')?.querySelector('[data-step-observation-input]');if(input?.value.trim())addStepObservation(b.dataset.plan,b.dataset.step,input.value)});
...
function planCard(p){const project=findProject(p.projectId);const total=p.steps.reduce((a,s)=>a+s.deliverables.length,0),done=p.steps.reduce((a,s)=>a+s.deliverables.filter(d=>d.done).length,0),pct=total?Math.round(done/total*100):0;return `<section class="card plan-card" data-plan-card-id="${p.id}"><div class="plan-head"><div><h3 ${window.ERPVisualIcons?.attributes(p.icon)||''}>${esc(p.title)}</h3><div class="plan-meta"><span class="badge">${esc(project?.name||'Sem projeto')}</span><span class="badge ${p.status==='Concluído'?'ok':''}">${esc(p.status)}</span><span class="badge">${pct}% dos entregáveis</span></div></div>${canManageCore()?`<div class="actions"><button class="btn icon secondary" data-edit-plan="${p.id}" title="Editar plano">✎</button><button class="btn icon danger" data-del-plan="${p.id}" title="Excluir plano">×</button></div>`:''}</div><div class="progress"><i style="width:${pct}%"></i></div>${p.steps.length?p.steps.map(s=>stepCard(p,s)).join(''):'<div class="notice">Este plano ainda não possui etapas.</div>'}${canManageCore()?`<div style="margin-top:12px"><button class="btn small secondary" data-add-step="${p.id}">+ Adicionar etapa</button></div>`:''}</section>`}
function stepCard(p,s){const editable=canEditStep(s);const names=s.responsibleIds.map(id=>findUser(id)?.name).filter(Boolean).join(', ')||'Sem responsáveis';const concluded=s.status==='Concluída';const d=daysUntil(s.deadline);const deadlineBadge=concluded?'<span class="badge ok">Concluído</span>':s.deadline?(d<0?'<span class="badge danger">Atrasada</span>':d<=7?'<span class="badge warn">Prazo próximo</span>':''):'';const pct=s.deliverables.length?Math.round(s.deliverables.filter(x=>x.done).length/s.deliverables.length*100):0;return `<div class="step collapsed ${editable?'':'readonly'}"><div class="step-top"><button type="button" class="step-toggle" data-toggle-step aria-expanded="false"><span class="step-chevron">⌄</span><span><span class="step-title" ${window.ERPVisualIcons?.attributes(s.icon)||''}>${esc(s.title)} ${deadlineBadge}</span><span class="muted">${editable?'Você pode atualizar esta etapa':'Somente visualização'} · ${pct}% concluído</span></span></button>${canManageCore()?`<div class="actions"><button class="btn icon secondary" data-plan="${p.id}" data-edit-step="${s.id}">✎</button><button class="btn icon danger" data-plan="${p.id}" data-del-step="${s.id}">×</button></div>`:''}</div><div class="step-details"><div class="step-info"><div class="info-box"><b>Início do Prazo</b>${brDate(s.startDeadline)}</div><div class="info-box"><b>Prazo</b>${brDate(s.deadline)}</div><div class="info-box"><b>Responsáveis</b>${esc(names)}</div><div class="info-box"><b>Progresso</b>${pct}%</div></div><div class="field"><label>Status</label>${editable?`<select data-step-status data-plan="${p.id}" data-step="${s.id}">${['Pendente','Em andamento','Aguardando','Concluída'].map(x=>`<option ${s.status===x?'selected':''}>${x}</option>`).join('')}</select>`:`<div class="badge">${esc(s.status)}</div>`}</div><div class="deliverables"><b>Entregáveis</b>${s.deliverables.length?s.deliverables.map(d=>`<label class="deliverable ${d.done?'done':''}"><input type="checkbox" data-deliverable="${d.id}" data-plan="${p.id}" data-step="${s.id}" ${d.done?'checked':''} ${editable?'':'disabled'}><span>${esc(d.text)}</span></label>`).join(''):'<div class="muted">Nenhum entregável cadastrado.</div>'}</div><div class="field step-observations"><label>Histórico de observações</label>${stepObservationsHtml(s)}${editable?`<div class="step-observation-compose"><textarea data-step-observation-input placeholder="Digite uma nova observação"></textarea><button type="button" class="btn secondary" data-add-step-observation data-plan="${p.id}" data-step="${s.id}">Salvar observação</button></div>`:''}</div></div></div>`}
...
function stepModal(plan,s={}){let deliverables=(s.deliverables||[]).map(d=>({...d}));openModal(s.id?'Editar etapa':'Nova etapa',`<form id="stepForm" class="form-grid"><div class="field full"><label>Nome da etapa</label><input name="title" required value="${esc(s.title||'')}"></div>${window.ERPVisualIcons?.pickerHtml(s.icon)||''}<div class="field"><label>Início do Prazo</label><input name="startDeadline" type="date" value="${s.startDeadline||''}"></div><div class="field"><label>Prazo</label><input name="deadline" type="date" value="${s.deadline||''}"></div><div class="field"><label>Status</label><select name="status">${['Pendente','Em andamento','Aguardando','Concluída'].map(x=>`<option ${s.status===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field full"><label>Responsáveis (selecione um ou mais)</label><div class="check-grid">${db.users.filter(u=>u.active&&!['Administrador','Diretor Técnico'].includes(u.type)).map(u=>`<label class="check-item"><input type="checkbox" name="responsibleIds" value="${u.id}" ${(s.responsibleIds||[]).includes(u.id)?'checked':''}>${esc(u.name)} <span class="muted">(${esc(u.type)})</span></label>`).join('')||'<div class="notice">Cadastre usuários operacionais primeiro.</div>'}</div></div><div class="field full"><label>Entregáveis</label><div class="inline-add"><input id="newDeliverable" placeholder="Digite um entregável"><button id="addDeliverable" class="btn secondary" type="button">Adicionar</button></div><div id="deliverableTags" class="tag-list"></div></div>${s.id?'<div class="field full"><div class="notice">As observações existentes são permanentes e novas observações devem ser adicionadas dentro da etapa.</div></div>':'<div class="field full"><label>Observação inicial (opcional)</label><textarea name="initialObservation"></textarea></div>'}</form>`,()=>$('#stepForm').requestSubmit());
  ...
  $('#stepForm').onsubmit=e=>{e.preventDefault();const fd=new FormData(e.target),responsibleIds=fd.getAll('responsibleIds');if(!responsibleIds.length){alert('Selecione pelo menos um responsável.');return}const f={title:fd.get('title'),icon:window.ERPVisualIcons?.choice(fd.get('icon'))??s.icon??'',startDeadline:fd.get('startDeadline'),deadline:fd.get('deadline'),status:fd.get('status'),responsibleIds,deliverables};if(f.startDeadline&&f.deadline&&f.startDeadline>f.deadline){alert('O Início do Prazo não pode ser posterior ao Prazo.');return}if(s.id)Object.assign(s,f);else{const initial=String(fd.get('initialObservation')||'').trim(),notes=initial?encodeStepObservations([{id:uid(),text:initial,author:currentUser?.name||currentUser?.email||'Usuário',userId:currentUser?.id||null,createdAt:new Date().toISOString()}]):'';plan.steps.push({id:uid(),...f,notes})};saveDB();closeModal();renderPlans()};
}
...
function metasCalendar(allSteps){
  ...
  const steps=allSteps.filter(s=>s.deadline&&s.status!=='Concluída');
...
function metasPlanCardHtml(p,userId){
  ...
  const nextDeadline=mySteps.filter(s=>s.status!=='Concluída').map(s=>s.deadline).filter(Boolean).sort()[0];
...
function metasChecklistHtml(plan){
  const items=plan.steps.length?plan.steps.map(s=>{
    ...
    return `<label class="deliverable metas-checklist-item ${s.status==='Concluída'?'done':''}"><input type="checkbox" data-metas-step-toggle="${s.id}" ${s.status==='Concluída'?'checked':''} ${editable?'':'disabled'}><span><strong>${esc(s.title)}</strong><small class="muted">${esc(names)}${s.deadline&&s.status!=='Concluída'?` · Prazo ${brDate(s.deadline)}`:''}</small></span></label>`;
...
function metasPrazosHtml(plan){
  ...
  return `<div class="metas-prazos-list">${sorted.map(s=>{
    const concluded=s.status==='Concluída';const d=daysUntil(s.deadline);const badge=concluded?'<span class="badge ok">Concluído</span>':s.deadline?(d<0?'<span class="badge danger">Atrasada</span>':d<=7?'<span class="badge warn">Prazo próximo</span>':''):'';
    return `<div class="metas-prazo-item"><div><strong>${esc(s.title)}</strong><span class="muted">${esc(s.status)}</span></div><div>${concluded?'':brDate(s.deadline)} ${badge}</div></div>`;
  }).join('')}</div>`;
}
...
window.ERPCoreNavigation={
  ...
  open(type,id){
    if(type==='plano'){
      ...
      if(!p||!canSeePlan(p))throw new Error('Plano indisponível para este usuário.');
      currentView='plans';currentPlanDetailId=id;renderApp();
    }else if(type==='projeto'){
      if(!db.projects.some(x=>x.id===id))throw new Error('Projeto indisponível para este usuário.');
      currentView='projects';currentProjectId=id;renderApp();
    }
  }
};
init();
...
function nextWeekMondayISO(){return iso(weekStartDate(1));}
...
function renderHome(){
  ...
  const week=visibleMetas(metasForWeek()).slice().sort((a,b)=>Number(a.status==='Concluído')-Number(b.status==='Concluído')||String(a.titulo).localeCompare(String(b.titulo),'pt-BR'));const active=visibleMetas().filter(m=>!['Concluído','Cancelado'].includes(m.status));const late=active.filter(m=>m.prazo&&m.prazo<today());
...
function metaCardHtml(m){const resp=metaResponsibles(m.id).map(userName).join(', ')||'Sem responsável';const action=conclusionActionHtml(m);return `<button type="button" class="metas2-card" data-meta-card="${m.id}"><div class="metas2-card-top"><strong ${window.ERPVisualIcons?.attributes(m.icone)||''}>${esc(m.titulo)}</strong>${statusBadge(m.status)}</div><span class="muted metas2-assoc">${esc(associationLabel(m))}</span><p>${esc(m.observacoes||'Sem observações.')}</p><div class="metas2-card-foot"><span>${esc(sectorLabel(m.setor_id))}</span><span>${m.prazo?'Prazo '+brDate(m.prazo):'Sem prazo'}</span></div><small>${esc(resp)}</small>${action?`<div class="meta-conclusion-row">${action}</div>`:''}</button>`}
function metaLateCardHtml(m){const resp=metaResponsibles(m.id).map(userName).join(', ')||'Sem responsável';const action=conclusionActionHtml(m);return `<button type="button" class="metas2-card metas2-card-late" data-meta-card="${m.id}"><div class="metas2-card-top"><strong ${window.ERPVisualIcons?.attributes(m.icone)||''}>${esc(m.titulo)}</strong><span class="badge danger">Atrasado</span></div><span class="muted metas2-assoc">${esc(associationLabel(m))}</span><p>${esc(m.observacoes||'Sem observações.')}</p><div class="metas2-card-foot"><span>${esc(sectorLabel(m.setor_id))}</span><span class="meta-late-deadline">${m.prazo?'Prazo '+brDate(m.prazo):'Sem prazo'}</span></div><small>${esc(resp)}</small>${action?`<div class="meta-conclusion-row">${action}</div>`:''}</button>`}
...
function userColumnsHtml(metas,late=false){const groups=new Map();metas.forEach(m=>{const ids=metaResponsibles(m.id);const id=ids[0]||'sem-responsavel';if(!groups.has(id))groups.set(id,[]);groups.get(id).push(m);});return `<div class="metas-user-columns">${[...groups.entries()].sort((a,b)=>userName(a[0]).localeCompare(userName(b[0]),'pt-BR')).map(([id,list])=>`<section class="metas-user-column"><header><strong>${esc(id==='sem-responsavel'?'Sem responsável':userName(id))}</strong><span>${list.length}</span></header><div>${list.map(late?metaLateCardHtml:metaCardHtml).join('')}</div></section>`).join('')||'<div class="empty">Nenhuma meta encontrada.</div>'}</div>`;}
function sectorPanelHtml(metas){return `<section class="card metas-sector-panel"><div class="section-head"><div><h3>Setores</h3><p class="muted">Crie, edite ou exclua setores usados na organização das metas.</p></div></div><div class="metas-group-list">${state.sectors.filter(s=>s.ativo!==false).map(s=>{const count=metas.filter(m=>m.setor_id===s.id).length;return `<div class="metas-group-item sector-manage-row"><button type="button" data-filter-sector="${s.id}"><strong>${esc(s.nome)}</strong><span class="muted">${count} meta(s)</span></button>${canManageSectors()?`<div class="actions"><button class="btn icon secondary" data-edit-sector="${s.id}">✎</button><button class="btn icon danger" data-del-sector="${s.id}">×</button></div>`:''}</div>`}).join('')||'<div class="empty compact">Nenhum setor.</div>'}</div></section>`;}
function wireSectorPanel(){qsa('[data-edit-sector]').forEach(b=>b.onclick=()=>sectorModal(state.sectors.find(s=>s.id===b.dataset.editSector)));qsa('[data-del-sector]').forEach(b=>b.onclick=()=>deleteSector(b.dataset.delSector));qsa('[data-filter-sector]').forEach(b=>b.onclick=()=>{const id=b.dataset.filterSector;qsa('.metas2-card').forEach(card=>{const m=state.metas.find(x=>x.id===card.dataset.metaCard);card.hidden=m?.setor_id!==id})});}
function renderActive(mode='all'){
  state.screen='active';const allActive=visibleMetas().filter(m=>!['Concluído','Cancelado'].includes(m.status));const late=mode==='late';const active=late?allActive.filter(m=>m.prazo&&m.prazo<today()):allActive;B().title(late?'Metas Atrasadas':'Metas Ativas');
  qs('#content').innerHTML=`<div class="toolbar"><button class="btn ghost" id="metaActiveBack">← Voltar</button><div class="right">${canManageSectors()?'<button class="btn secondary" id="metaNewSector">+ Setor</button>':''}${canManageMeta()?'<button class="btn" id="metaActiveNew">+ Nova Meta</button>':''}</div></div><section class="card metas-user-board ${late?'metas-user-board-late':''}"><div class="section-head"><div><h3>${late?'Metas atrasadas':'Metas em aberto'}</h3><p class="muted">${late?'Organizadas por responsável, com os prazos vencidos em destaque.':'Organizadas por responsável para facilitar o acompanhamento.'}</p></div></div>${userColumnsHtml(active,late)}</section>${sectorPanelHtml(active)}`;
  qs('#metaActiveBack').onclick=renderHome;qs('#metaNewSector')?.addEventListener('click',()=>sectorModal());qs('#metaActiveNew')?.addEventListener('click',()=>metaModal());wireMetaCards();wireSectorPanel();
}
...
async function metaModal(prefill=null,defaultUserId=null){if(B()?.refreshCore)await B().refreshCore();const m=prefill?.id?prefill:null;const checklistInicial=m?checklistItems(m.id).filter(x=>!x.concluido).map(x=>x.titulo):[];const assocType=m?.associacao_tipo||prefill?.associacao_tipo||'avulsa';const assocId=m?.associacao_id||prefill?.associacao_id||'';const resp=m?metaResponsibles(m.id):(defaultUserId?[defaultUserId]:[]);B().openModal(m?'Editar meta':'Nova Meta',`<form id="metaV2Form" class="form-grid"><div class="field full"><label>Título da nova meta</label><input name="titulo" required value="${esc(m?.titulo||'')}"></div>${window.ERPVisualIcons?.pickerHtml(m?.icone,'titulo')||''}<div class="field full"><label>Observações</label><textarea name="observacoes" placeholder="Contexto, instruções ou observações da meta">${esc(m?.observacoes||'')}</textarea></div><div class="field full"><label>Checklist</label><div id="metaChecklistDraft">${(checklistInicial.length?checklistInicial:['']).map(t=>`<label class="meta-checklist-draft-item"><input type="checkbox" disabled><input data-checklist-draft value="${esc(t)}" placeholder="Descreva o item"></label>`).join('')}</div><button type="button" class="btn small secondary" id="metaChecklistDraftAdd">+ Adicionar item</button><small class="muted">Adicione os itens que serão marcados durante a execução.</small></div><div class="field"><label>Semana</label><input name="semana" type="date" value="${m?.semana_inicio||nextWeekMondayISO()}"></div><div class="field"><label>Prazo (opcional)</label><input name="prazo" type="date" value="${m?.prazo||(m?'' : nextWeekMondayISO())}"></div><div class="field"><label>Status</label><select name="status">${META_STATUS.map(x=>`<option ${(m?.status||'Em andamento')===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>Setor</label><select name="setor"><option value="">Sem setor</option>${state.sectors.filter(s=>s.ativo!==false).map(s=>`<option value="${s.id}" ${m?.setor_id===s.id?'selected':''}>${esc(s.nome)}</option>`).join('')}</select></div><div class="field full"><label>Associar meta a</label><select name="assocType" id="metaAssocType"><option value="plano" ${assocType==='plano'?'selected':''}>Plano de Trabalho</option><option value="projeto" ${assocType==='projeto'?'selected':''}>Projeto do ERP</option><option value="ordem_servico" ${assocType==='ordem_servico'?'selected':''}>Ordem de Serviço</option><option value="avulsa" ${assocType==='avulsa'?'selected':''}>Meta avulsa</option></select></div><div class="field full" id="metaAssocWrap"></div><div class="field full"><label>Responsáveis</label><div class="check-grid">${db().users.filter(u=>u.active&&allowedProfessional(u)).map(u=>`<label class="check-item"><input type="checkbox" name="responsavel" value="${u.id}" ${resp.includes(u.id)?'checked':''}>${esc(u.name)} <span class="muted">(${esc(u.type)})</span></label>`).join('')}</div></div></form>`,()=>qs('#metaV2Form').requestSubmit());const drawAssoc=()=>{const type=qs('#metaAssocType').value;qs('#metaAssocWrap').innerHTML=type==='avulsa'?'<div class="notice">Meta avulsa: o histórico ficará registrado no card de cada colaborador responsável.</div>':`<label>${type==='plano'?'Plano de Trabalho':type==='projeto'?'Projeto do ERP':'Ordem de Serviço'}</label><select name="assocId" required><option value="">Selecione</option>${associationOptions(type,assocId)}</select>`};drawAssoc();qs('#metaAssocType').onchange=drawAssoc;qs('#metaChecklistDraftAdd').onclick=()=>{const row=document.createElement('label');row.className='meta-checklist-draft-item';row.innerHTML='<input type="checkbox" disabled><input data-checklist-draft placeholder="Descreva o item">';qs('#metaChecklistDraft').appendChild(row);row.querySelector('[data-checklist-draft]').focus();};qs('#metaV2Form').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),responsaveis=fd.getAll('responsavel');if(!responsaveis.length){alert('Selecione ao menos um responsável.');return}const type=fd.get('assocType'),aid=type==='avulsa'?null:fd.get('assocId');if(type!=='avulsa'&&!aid){alert('Selecione o item que será associado à meta.');return}const row={id:m?.id||uid(),titulo:fd.get('titulo').trim(),icone:(window.ERPVisualIcons?.choice(fd.get('icon'))??m?.icone)||null,observacoes:fd.get('observacoes').trim()||null,semana_inicio:mondayForDate(fd.get('semana')),prazo:fd.get('prazo')||null,status:fd.get('status')||'Em andamento',setor_id:fd.get('setor')||null,associacao_tipo:type,associacao_id:aid||null,created_by:m?.created_by||currentUser().id,updated_at:new Date().toISOString()};const save=await sb().from('metas').upsert(row,{onConflict:'id'}).select().single();if(save.error){alert(save.error.message);return}const id=row.id;const del=await sb().from('meta_responsaveis').delete().eq('meta_id',id);if(del.error){alert(del.error.message);return}const ins=await sb().from('meta_responsaveis').insert(responsaveis.map(usuario_id=>({meta_id:id,usuario_id})));if(ins.error){alert(ins.error.message);return}const checklist=qsa('[data-checklist-draft]').map(x=>x.value.trim()).filter(Boolean);const delCk=await sb().from('meta_checklist').delete().eq('meta_id',id).eq('concluido',false);if(delCk.error){alert(delCk.error.message);return}if(checklist.length){const ckIns=await sb().from('meta_checklist').insert(checklist.map(titulo=>({id:uid(),meta_id:id,titulo,created_by:currentUser().id})));if(ckIns.error){alert(ckIns.error.message);return}}if(m){Object.assign(m,save.data);state.responsibles=state.responsibles.filter(r=>r.meta_id!==id)}else state.metas.unshift(save.data);state.responsibles.push(...responsaveis.map(usuario_id=>({meta_id:id,usuario_id})));await historyAdd(save.data,{acao:m?'Editada':'Criada',descricao:m?'Dados da meta atualizados.':'Meta criada e atribuída.'});B().closeModal();state.screen==='orders'&&type==='ordem_servico'?orderDetail(aid):renderHome()}}
