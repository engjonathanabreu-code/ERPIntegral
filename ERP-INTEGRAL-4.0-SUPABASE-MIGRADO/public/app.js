...
const USER_TYPES=['Administrador','Comercial','Projetos','Topografia','Marketing','Pós-protocolo','Atendimentos'];
...
function canSeeMetas(){return isAdmin()||isMetasSector();}
...
function navItems(){if(isAdmin())return ADMIN_NAV;if(isComercial())return COMERCIAL_NAV;if(isMetasSector())return [['plans','Planos de trabalho'],['metas','Metas']];return [['plans','Planos de trabalho']];}
function renderApp(){
  matrixStop();
...
function renderMetas(){
  ...
  else if(metasScreen==='ativas'&&isAdmin())renderMetasAtivas();
...
function renderMetasHome(){
  title('Metas');
  const employees=isAdmin()?metasScopeUsers():[db.users.find(u=>u.id===currentUser.id)||{id:currentUser.id,name:currentUser.name,type:currentUser.type}];
  ...
  $('#metasAtivasBtn').onclick=()=>{if(isAdmin()){metasScreen='ativas';metasSectorFilter=null;}else{metasBoardUser=currentUser.id;metasScreen='board';}renderMetas()};
...
function renderMetasBoard(){
  ...
  $('#content').innerHTML=`<div class="toolbar"><button id="metasBack" class="btn ghost">← Voltar</button><div class="right"><span class="badge">${esc(u.type)}</span></div></div>${metasKpiHtml(stepsForUser(u.id))}<div class="metas-board">${PLAN_STATUS_COLUMNS.map(col=>{
  ...
  $('#metasBack').onclick=()=>{metasScreen=isAdmin()?'ativas':'home';renderMetas()};
...
function metasChecklistHtml(plan){
  if(!plan.steps.length)return '<div class="empty compact">Este plano ainda não possui etapas.</div>';
