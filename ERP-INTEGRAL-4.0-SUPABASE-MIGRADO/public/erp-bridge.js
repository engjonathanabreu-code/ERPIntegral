...
async function loadCore(){if(!sb)return;try{const {data:{user}}=await sb.auth.getUser();if(!user)return;const [pr,ur,pp,st,rr]=await Promise.all([sb.from('profiles').select('*').order('nome'),sb.from('projetos').select('*').order('created_at'),sb.from('planos_trabalho').select('*').order('created_at'),sb.from('etapas_plano').select('*').order('ordem'),sb.from('etapa_responsaveis').select('*')]);if(pr.error||ur.error||pp.error)return;core.users=(pr.data||[]).map(x=>({id:x.id,name:x.nome,email:x.email||'',type:x.tipo,sector:x.setor||x.tipo,active:x.ativo!==false}));core.projects=(ur.data||[]).map(x=>({id:x.id,name:x.nome,type:x.tipo_servico,clientId:x.cliente_id||''}));const steps=st.data||[],resp=rr.data||[];core.plans=(pp.data||[]).map(x=>({id:x.id,title:x.titulo,projectId:x.projeto_id||'',status:x.status,steps:steps.filter(s=>s.plano_id===x.id).map(s=>({id:s.id,title:s.titulo,responsibleIds:resp.filter(r=>r.etapa_id===s.id).map(r=>r.usuario_id)}))}));me=core.users.find(x=>x.id===user.id)||{id:user.id,name:user.email,type:'',sector:'',active:true};window.dispatchEvent(new CustomEvent('erp-bridge-ready'));installMetasNav();tagPlanCards()}catch(e){console.warn('ERP bridge',e)}}
