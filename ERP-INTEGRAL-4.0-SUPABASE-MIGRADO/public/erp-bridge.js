...
function installDirectorMetasNav(){if(!me||!['Diretor de Projetos'].includes(me.type))return;const nav=document.querySelector('.nav');if(!nav||nav.querySelector('[data-standalone-metas]')||nav.querySelector('[data-view="metas"]'))return;const b=document.createElement('button');b.dataset.standaloneMetas='metas';b.textContent='Metas';b.onclick=()=>{nav.querySelectorAll('button').forEach(x=>x.classList.remove('active'));b.classList.add('active');window.ERPMetasV2?.render?.()};nav.appendChild(b)}
function hookMetasButton(){document.querySelectorAll('.nav [data-view="metas"]').forEach(b=>{if(b.dataset.v2hook)return;b.dataset.v2hook='1';b.addEventListener('click',()=>setTimeout(()=>window.ERPMetasV2?.render?.(),20))})}
...
const observer=new MutationObserver(()=>{installTypeOption();installDirectorMetasNav();hookMetasButton();hookProjects();if(activeView()==='metas')setTimeout(()=>window.ERPMetasV2?.render?.(),0)});observer.observe(document.documentElement,{childList:true,subtree:true});
window.ERPIntegralBridge={get db(){return core},get currentUser(){return me},get currentView(){return activeView()},get currentProjectId(){return lastProjectId},sb,esc,uid,today,brDate,money,title,openModal,closeModal,findUser:id=>core.users.find(x=>x.id===id),findProject:id=>core.projects.find(x=>x.id===id),findClient:()=>null,cacheDB:()=>{},saveDB:()=>{}};
loadCore();hookMetasButton();hookProjects();
})();
